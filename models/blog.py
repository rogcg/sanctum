...
class Post(db.Model):
    ...
    def get_delete_url(self):
        return "/admin/post/delete/%04d/%02d/%02d/%s" % (self.pub_date.year,
                                                       self.pub_date.month,
                                                       self.pub_date.day,
                                                       self.slug)
...
class Page(db.Model):
    ...
    def get_edit_url(self):
        return "/admin/page/edit/%s" % self.url

    def get_delete_url(self):
        return "/admin/page/delete/%s" % self.url

    def put(self):

        self.test_for_url_collision()
        self.populate_html_fields()
    ...
    def test_for_url_collision(self):

        page = Page.all(keys_only=True).filter('url =', self.url).get()

        # If any url matches were found then an exception should be raised
        if page and (not self.is_saved() or self.key() != page):
            raise URLConstraintViolation(self.url)
...
class URLConstraintViolation(Exception):
    def __init__(self, url):
        super(URLConstraintViolation, self).__init__("URL '%s' is not unique." % url)